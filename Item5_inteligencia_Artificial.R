...
dados_RF<-dados_IA%>%dplyr::select(c(4, 8:25))
View(dados_RF)
...
dados_SVM <- dados_IA%>%dplyr::select(c(4, 8:25))
View(dados_SVM)
...
#Modelo SVM
SVM<-svm(formula=dados_SVM$volume_total~.,data=dados_SVM,type = 'eps-regression', kernel = 'radial')
...
#Atribuindo o arquivo para dados_SVM
dados_RNA <- dados_IA%>%dplyr::select(c(4,8:25))
View(dados_RNA)
...
#Atribuindo o arquivo para dados_SVM
dados_KNN <- dados_IA%>%dplyr::select(c(4,8:25))
View(dados_KNN)
...
ddadosLAS_norm<-readLAS("Item_5_Dados_LiDAR/las_normalizada.las", select = "xyz",filter = " -drop_z_below 0")
